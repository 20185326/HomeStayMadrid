...
let CoreClass$1 = class CoreClass {
    ...
    databaseClass
    providerClass
    generalArgs = { blackList: [],endFlowList: [], listEvents: {}, delay: 0 }
    ...
    listenerBusEvents = () => [
        ...
        {
            event:'menssageFromBot',
            func:(msgB)=>{
                if(!this.generalArgs.endFlowList.includes(msgB.from)){
                    this.generalArgs.endFlowList.push(msgB.from)
                }
                if(msgB.body==="Gracias"){
                    this.generalArgs.endFlowList=this.generalArgs.endFlowList.filter(item=>item!==msgB.from)
                }
            }
        }
    ]
    ...
    handleMsg = async (messageCtxInComming) => {
        ...
        if(this.generalArgs.endFlowList.includes(from))return
